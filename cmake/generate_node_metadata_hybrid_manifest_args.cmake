...
#   forwarded to auto_apms_behavior_tree_generate_node_metadata to generate a C++ header
      ...
      if("${_index}" GREATER -1)
        ...
        if("${_index}" GREATER -1)
          ...
          if("${_index}" GREATER -1)
            ...
            message(
              FATAL_ERROR
              "auto_apms_behavior_tree_generate_node_metadata_hybrid_manifest_args(): Metadata ID '${_var}' was provided, but no such resource can be found."
            )
          endif()
        endif()
        ...
        if(NOT EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${_var}")
          message(
            FATAL_ERROR
            "auto_apms_behavior_tree_generate_node_metadata_hybrid_manifest_args(): '${_var}' is interpreted as a relative path to a node manifest, but no such file can be found relative to the current source directory ${CMAKE_CURRENT_SOURCE_DIR}."
          )
        endif()
      endif()
    endforeach()
      ...
      if("${_generate_metadata_inputs_length}" EQUAL "${_matching_existing_metadata_count}")
        ...
        foreach(_node_manifest_path ${_generate_metadata_inputs})
          if("${_node_manifest_path}" IN_LIST _existing_node_manifest_abs_paths__build)
            ...
            list(APPEND _auto_apms_behavior_tree_generate_node_metadata_hybrid_manifest_args__node_manifest_rel_paths__install "${_AUTO_APMS_BEHAVIOR_TREE_CORE__RESOURCE_DIR_RELATIVE__METADATA}/${_file_name}")
          else()
            # Regarding the manifests generated by other package, we use the existing absolute manifest install paths.
            list(APPEND _auto_apms_behavior_tree_generate_node_metadata_hybrid_manifest_args__node_manifest_rel_paths__install "${_node_manifest_path}")
          endif()
        endforeach()
      else()
        ...
        auto_apms_behavior_tree_generate_node_metadata("${metadata_id}" ${_generate_metadata_inputs} ${_forward_args})
