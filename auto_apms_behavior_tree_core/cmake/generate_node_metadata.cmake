...
#   - Optionally, a C++ header that defines model classes for all behavior tree nodes.
#
...
# process, it's verified that the node plugins are registered correctly.
...
# :param ARGN: One or paths to node manifests. If relative, must be relative to CMAKE_CURRENT_SOURCE_DIR.
...
#   nodes specified inside the node manifest files provided under ARGN and
...
macro(auto_apms_behavior_tree_generate_node_metadata metadata_id)
  ...
  foreach(_input_manifest_file_path ${ARGS_UNPARSED_ARGUMENTS})
    ...
    if(NOT EXISTS "${_input_manifest_file_path__source}")
      message(
          FATAL_ERROR
          "auto_apms_behavior_tree_generate_node_metadata(): Node manifest file '${_input_manifest_file_path}' doesn't exist."
      )
    endif()
  ...
  # Simultaneously, define a variable containing the library paths and generator expressions for node plugin libraries
  # that need to be added as dependencies.
...
${_error}"
  ...
  if(NOT "${ARGS_NODE_MODEL_HEADER_TARGET}" STREQUAL "")
    ...
    # IMPORTANT: The target NODE_MODEL_HEADER_TARGET should link against auto_apms_behavior_tree_core
    # (auto_apms_behavior_tree is also ok, because it recursively depends on auto_apms_behavior_tree_core), because otherwise
...
Otherwise, building downstream packages won't work unless they add this dependency themselves. Downstream CMake will fail since it cannot resolve the #include statements inside the generated header."
)
    endif()
  ...
  # Store the metadata information for reusing it during auto_apms_behavior_tree_register_trees()
  list(APPEND _AUTO_APMS_BEHAVIOR_TREE_CORE__NODE_MANIFEST_BUILD_INFO "${metadata_id}@${_generated_node_manifest_abs_path__build}")
  set(_AUTO_APMS_BEHAVIOR_TREE_CORE__RESOURCE_FILE__NODE_MANIFEST "${_AUTO_APMS_BEHAVIOR_TREE_CORE__RESOURCE_FILE__NODE_MANIFEST}${metadata_id}|${_generated_node_manifest_rel_dir__install}/${_generated_node_manifest_file_name}|${_generated_node_model_rel_dir__install}/${_generated_node_model_file_name}\n")
